...
function getStateOfChat() {
	if(!instanse){
		...
		$.ajax({
			...
			crossDomain: true,
			cache: false,
			url: "https://www.dfunstation.com/api4/consultant/index.php/member/materi/"+userid+"/"+localStorage.userid+"//"+chatid+"/?callback=?",
...
function updateChatx() {
	if(!instanse){
		...
		var idx = getvar("id");
		if(idx != "") {
			var urlx = "https://www.dfunstation.com/api4/consultant/index.php/member/materi/"+userid+"/"+localStorage.userid+"/"+idx+"/"+chatid+"/?callback=?";
		} else {
			var urlx = "https://www.dfunstation.com/api4/consultant/index.php/member/materi/"+userid+"/"+localStorage.userid+"/0/"+chatid+"/?callback=?";
		}
		$.ajax({
			type: "POST",
			crossDomain: true,
	...
	if(!instanse){
		...
		if (length <= maxLength + 1) { 
			$.ajax({
				...
				crossDomain: true,
				cache: false,
				url: "https://www.dfunstation.com/api4/consultant/index.php/member/materi/"+userid+"/"+localStorage.userid+"/"+id+"/"+id+"/?callback=?",
...
function like(id) {
	...
	if(!instanse){
		...
		$.ajax({
			...
			crossDomain: true,
			cache: false,
			url: "https://www.dfunstation.com/api4/consultant/index.php/member/materi/"+userid+"/"+localStorage.userid+"/"+id+"/"+chatid+"/?callback=?",
...
function sendChat(message, nickname) { 
	...
	var isimagex = $("#isimagex").val();
	//alert('isimage' + isimagex);
	if($.trim(chatid).length>0){
		if(isimage_=="1") {
			...
			ft.upload(imageURI, encodeURI("https://www.dfunstation.com/api4/consultant/index.php/member/media-materi/"+localStorage.userid+"/?callback=?"), win, fail, options);
		} else if(isimage=="1") {
			...
			//alert('isimagex');
			$("#kirimx").html('Mengirimkan...');
			var imageURI = document.getElementById('files').getAttribute("href");
			if (!imageURI) {
				swal('File PDF masih kosong');
				return;
			}
			var options = new FileUploadOptions();
				options.fileKey = "file";
