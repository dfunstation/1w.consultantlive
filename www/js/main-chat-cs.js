...
function getStateOfChat() {
	if(!instanse){
		...
		$.ajax({
			...
			crossDomain: true,
			cache: false,
			url: "https://www.dfunstation.com/api4/consultant/index.php/member/chatkonsultan/"+chatid+"/"+localStorage.userid+"/?callback=?",
...
function updateChatx() {
	...
	if(!instanse){
		...
		$.ajax({
			...
			crossDomain: true,
			cache: false,
			url: "https://www.dfunstation.com/api4/consultant/index.php/member/chatkonsultan/"+chatid+"/"+localStorage.userid+"/"+localStorage.lastchatid+"/?callback=?",
			...
			success: function(data) {
				...
				if(data.finish == 1 && actionx =="read") {
					...
					slide(refx);	 */				
					window.location.href= 'konsultan_chat-finish-cs.html?action=finish&chatid='+chatid+'&userid='+localStorage.userid;
				}
			}
		});
	}
...
function sendChat(message, nickname) { 
	...
	if($.trim(chatid).length>0){
		if(isimage_=="1") {
			...
			ft.upload(imageURI, encodeURI("https://www.dfunstation.com/api4/consultant/index.php/member/chat-media/"+localStorage.userid+"/?callback=?"), win, fail, options);
		} else if(isimage=="1") {
